...
Desktop packaging also sanitizes the packaged macOS `.app` bundle in an `afterPack` step (xattrs plus Finder detritus files like `.DS_Store` / `._*`) before signing so metadata detritus does not break `codesign`.
