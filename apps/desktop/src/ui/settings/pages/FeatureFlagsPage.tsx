  ...
  return (
    <Card className="border-border/80 bg-card/85">
      <CardHeader>
        ...
        <CardDescription>
          These development flags are global across all workspaces. Some toggles require a restart
          to fully apply.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {FEATURE_FLAG_IDS.map((flagId) => {
          ...
          return (
            <div
              key={flagId}
              className="flex items-start justify-between gap-4 max-[960px]:flex-col"
            >
              <div>
                ...
                {forcedOffInPackaged ? (
                  <div className="mt-1 text-xs text-warning">Unavailable in packaged builds.</div>
                ) : null}
