...
| Term | Meaning |
| --- | --- |
| `workspaceRoot` | The directory that contains the project `.cowork/` directory. This is the anchor for project-level config, memory, and MCP overrides. |
...
`workingDirectory` is intentionally not the same thing as `workspaceRoot`. A session can run inside a subdirectory of the workspace, or even outside the workspace root, while still keeping project-level `.cowork/` data anchored at `workspaceRoot`.
...
## AGENTS.md Versus .cowork/AGENT.md
...
- `.cowork/AGENT.md` is project hot-cache memory. It is loaded by the memory system and is separate from hierarchical project instructions.
...
- Use `workspaceRoot` when reasoning about project-level `.cowork/` overrides.
