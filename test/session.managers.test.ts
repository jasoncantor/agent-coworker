...
function makeBaseContext(): SessionContext {
  return {
    ...
    deps: {
      connectProviderImpl: async () =>
        ({ ok: true, provider: "google", mode: "api_key", message: "ok" }) as any,
      ...
      sessionBackupFactory: async () =>
        ({ getPublicState: () => ({}), reloadFromDisk: async () => ({}) }) as any,
      harnessContextStore: {
        get: () => null,
      ...
      generateSessionTitleImpl: async () => ({
        title: "Generated",
        source: "heuristic",
        model: null,
      }),
...
describe("session managers", () => {
  ...
  test("SessionAdminManager listSessions hides the active idle session when other meaningful sessions exist", async () => {
    ...
    context.deps.getLiveSessionSnapshotImpl = (sessionId) =>
      sessionId === "session-1"
        ? {
            sessionId: "session-1",
            title: "New Session",
