...
export const openAiContinuationStateSchema = z.object({
...
export function supportsOpenAiContinuation(provider: unknown): provider is OpenAiContinuationProvider {
  return typeof provider === "string" && (OPENAI_CONTINUATION_PROVIDER_NAMES as readonly string[]).includes(provider);
}
...
export function continuationMatchesTarget(
  ...
  if (target.provider === "codex-cli" && state.accountId && target.accountId && state.accountId !== target.accountId) {
    return false;
  }
  return true;
}
