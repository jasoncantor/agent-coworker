...
const OPENAI_PROXY_FETCH_PATCH_STATE = Symbol.for("cowork.openaiProxy.fetchPatchState");
...
type OpenAiProxyFetchPatchState = {
  refCount: number;
  originalFetch: typeof fetch;
};
...
async function withPatchedOpenAiProxyFetch<T>(
  ...
  const globalWithState = globalThis as typeof globalThis & {
    [OPENAI_PROXY_FETCH_PATCH_STATE]?: OpenAiProxyFetchPatchState;
  };
  const existingState = globalWithState[OPENAI_PROXY_FETCH_PATCH_STATE];
  if (existingState) {
    existingState.refCount += 1;
    try {
      return await run();
      ...
      if (existingState.refCount === 0) {
        globalThis.fetch = existingState.originalFetch;
        delete globalWithState[OPENAI_PROXY_FETCH_PATCH_STATE];
      }
    }
  }
  ...
  globalThis.fetch = wrappedFetch as typeof fetch;
  globalWithState[OPENAI_PROXY_FETCH_PATCH_STATE] = {
    refCount: 1,
    originalFetch,
  };
  ...
  try {
    ...
    const currentState = globalWithState[OPENAI_PROXY_FETCH_PATCH_STATE];
    if (currentState) {
      currentState.refCount -= 1;
      if (currentState.refCount === 0) {
        globalThis.fetch = currentState.originalFetch;
...
export async function resolvePiModel(params: RuntimeRunTurnParams): Promise<ResolvedPiRuntimeModel> {
  ...
  if (provider === "aws-bedrock-proxy") {
    const baseUrl = resolveAwsBedrockProxyBaseUrl({ config: params.config });
    if (!baseUrl) {
      throw new Error(
