...
export function formatAwsBedrockProxyDiscoveryFailure(
  ...
  if (failure.code === "missing_base_url") {
    return "Set the global AWS Bedrock Proxy URL before saving a proxy token.";
  }
...
export function resolveAwsBedrockProxyBaseUrl(opts: {
  ...
  const configValue = opts.config?.awsBedrockProxyBaseUrl || (opts.config as { openaiProxyBaseUrl?: string } | undefined)?.openaiProxyBaseUrl;
  if (configValue) {
...
export async function discoverAwsBedrockProxyModelsDetailed(opts: {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const detail = parseDiscoveryErrorBody(rawText)
        ?? trimMessage(response.statusText || "");
      return {
        ok: false,
